...
with st.sidebar:
    ...
    temp = st.number_input("Temperature (°C)", value=22)[cite: 90]
    hum = st.number_input("Humidity (%)", value=46)[cite: 91]
    occ = st.selectbox("Occupancy", ["OCCUPIED", "EMPTY"], index=0)[cite: 92]
    tod = st.selectbox("Time of Day", ["MORNING", "AFTERNOON", "EVENING", "NIGHT"], index=3)[cite: 93]
    win = st.checkbox("Windows Open", value=False)[cite: 94]
